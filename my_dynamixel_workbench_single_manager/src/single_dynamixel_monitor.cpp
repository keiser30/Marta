...
#include "../include/my_dynamixel_workbench_single_manager/single_dynamixel_monitor.h"
...
void SingleDynamixelMonitor::shutdownSingleDynamixelMonitor(void)
{
	...
	{
		for(int j = 0; j < dxl_cnt[i]; j++)
			dynamixel_driver_[i]->writeRegister(vdexl_id_[i][j], "Torque_Enable", false); 
	}
  ros::shutdown();
}
...
{
	dxl_id_ = req.dxl_id;
	if(req.port >= PORT_NUM){
		for(int i = 0; i < PORT_NUM; i++)	
		{
			for(int j = 0; j < dxl_cnt[i]; j++)
			{
				if(dxl_id_ == vdexl_id_[i][j])
				{
					port_num = i;
					j = dxl_cnt[i];
					i = PORT_NUM;
